...
namespace SingleNet
{
    ...
    public:
        BaseActivation(const std::function<T(T)> &activation,
                       const std::function<T(T)> &activationDerivative)
            : m_activation(activation),
              m_activationDerivative(activationDerivative) {}

        BaseActivation(
            const std::function<Vector<T, 1>(Vector<T, 1>)> &activation,
            const std::function<Vector<T, 1>(Vector<T, 1>)> &activationDerivative)
            : m_activationVector(activation),
              m_activationDerivativeVector(activationDerivative) {}
    ...
    {
    public:
        Dense(size_t inputSize, size_t outputSize) : BaseNet(inputSize, outputSize)
        {
            ...
            try
            {
                ...
                for (size_t i = 0; i < shape(nextDelta)[0]; i++)
                    for (size_t j = 0; j < shape(nextDelta)[1]; j++)
                        result[i][j] =
                            m_activationDerivative(layerInput[i][j]) * nextDelta[i][j];
                                  ...
                                  const float &learningRate) override
        {
            try
            {
                ...
                for (size_t i = 0; i < shape(nextDelta)[0]; i++)
                    for (size_t j = 0; j < shape(nextDelta)[1]; j++)
                        result[i][j] =
                            m_activationDerivative(layerOutput[i][j]) * nextDelta[i][j];
    ...
    public:
        explicit SimpleRNN(size_t size)
            : BaseActivation(
                  [](const float &f)
                  {
                      return (2.0f / (1.0f + std::exp(-2.0f * f))) - 1.0f;
                  },
                  [](const float &f)
                  { return 1 - f * f; }),
              m_size(size),
                    ...
                    const float &learningRate)
        {
            ...
            for (size_t i = 0; i < shape(outputVector)[0]; i++)
                for (size_t j = 0; j < shape(outputVector)[1]; j++)
                    error += -(output[i][j] * std::log(outputVector[i][j]) +
                               (1.0f - output[i][j]) * std::log(1.0f - outputVector[i][j]));
